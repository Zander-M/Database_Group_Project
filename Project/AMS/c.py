...
def confirm_order():
    ...
    if request.form['type'] == 'search':
        ...
        cursor.execute(
            "SELECT * FROM flight WHERE flight_id = %s", (g.flight_id,))
        ...
        if ticket_sold / seat >= 0.7:
            ...
            price = base_price
            result = [flight[1], flight[7], flight[5],
                      flight[8], flight[6], price, flight[0]]
        ...
        cursor.execute(
            "SELECT * FROM flight WHERE flight_id = %s", (flight_id,))
        ...
        if error is None:
            try:
                cursor.execute("INSERT INTO ticket (flight_id, airline, customer_email, sold_price, payment_method, card_number, name_on_card, expiration_date, purchase_date_time) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,CURTIME())", (
                    flight[0], flight[1], g.user[0], price, payment, card_number, name_on_card, exp_date))
                cursor.execute(
                    "SELECT * FROM ticket WHERE flight_id = %s", flight[0])
                ticket_sold = len(cursor.fetchall())
                # find out how many seats are available
